...
//
// The plans screen and the Shopify subscription it starts.
//
// A zero-priced plan is recorded straight away -- there is nothing to charge,
// so there is nothing to approve. A priced plan goes out to Shopify, which
// shows the merchant an approval screen and returns to confirmPlan().
...
exports.postSelectPlan = async (req, res) => {
  ...
  try {
    ...
    // Free: cancel whatever is being paid for, then record the change now.
    ...
    // Paid: a development store gets a Shopify TEST charge so nobody is
    // billed for real while the app is being built or reviewed.
 ...
 * not carry an App Bridge token. The signed, expiring billing token pins the
 * callback to the exact store and plan instead.
 */
exports.confirmPlan = async (req, res) => {
  ...
  try {
    ...
    // Shopify currently appends charge_id to the return URL. The local pending
    // row is the fallback, and also lets this survive a callback change.
    const chargeId =
